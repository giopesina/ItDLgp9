...
def interval_mapping(image, from_min, from_max, to_min, to_max):
    ...
    # image: input array
    image = np.clip(image, from_min, from_max)
...
def image_generator(
    model_path, device, nz, ngf, nc, ngpu, num_samples, input_latent_vector=None
):
    ...
    if input_latent_vector is None:
        ...
        z = torch.from_numpy(input_latent_vector).to(device)
...
def generate(
    ...
    try:
        ...
        if device == "cuda":
            ngpu = 1
        image_list = image_generator(
            model_file, device, 100, 64, 1, ngpu, num_samples, input_latent_vector
        )
        if save_images:
